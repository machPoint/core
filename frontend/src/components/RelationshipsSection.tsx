...
export default function RelationshipsSection() {
  ...
  const handleAnalyzeRelationships = async () => {
    ...
    try {
      ...
      toast.success(`Discovered ${relationships.length} cross-system relationship${relationships.length !== 1 ? 's' : ''}`);
    } catch (error) {
      console.error('Error analyzing relationships:', error);
      toast.error(`Failed to analyze relationships: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 p-2 bg-muted/50 rounded text-xs">
                            {relationship.sourceA.title}
                          </div>
                          <ArrowRight className="w-4 h-4 text-muted-foreground flex-shrink-0" />
                          <div className="flex-1 p-2 bg-muted/50 rounded text-xs">
                            {relationship.sourceB.title}
                          </div>
                        </div>
                      </div>
