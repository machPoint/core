...
const CriticalPathAnalyzer: React.FC<CriticalPathAnalyzerProps> = ({
  requirements = ["REQ-FCS-001", "REQ-NAV-002", "REQ-HYD-003"],
  className
}) => {
  ...
  const criticalPaths = useMemo((): CriticalPath[] => {
    ...
    Object.entries(requirementGroups).forEach(([reqId, nodes]) => {
      ...
      const riskScore = criticalNodes.reduce((sum, n) => sum + n.risk, 0) / criticalNodes.length;
      const bottlenecks = criticalNodes.filter(n => n.slack === 0).map(n => n.id);
      const completion = criticalNodes.filter(n => n.status === 'completed' || n.status === 'verified').length / criticalNodes.length * 100;
  ...
  const resourceConstraints = useMemo((): ResourceConstraint[] => {
    ...
    return teams.map(team => {
      ...
      const allocation = Math.min(100, (totalEffort / (capacity * timeHorizon)) * 100);
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">
                {Math.floor(criticalPaths.reduce((sum, p) => sum + p.riskScore, 0) / Math.max(criticalPaths.length, 1))}
              </div>
